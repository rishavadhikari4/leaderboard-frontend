...
export function NestSmsSalesSection() {
    ...
    const fetchTodayData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setTransactions(data.filter((t) => t.source === "sms").reverse());
        }
      } catch (err) {
        console.error("Error fetching Nest SMS transactions", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="text-center text-sm text-[#0e0e0e]/40 py-8">
        Loading Nest SMS sales...
      </div>
    );
  }

  return <SourceColumn source="sms" transactions={transactions} />;
}
