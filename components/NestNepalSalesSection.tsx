...
export function NestNepalSalesSection() {
    ...
    const fetchTodayData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setTransactions(data.filter((t) => t.source === "nest").reverse());
        }
      } catch (err) {
        console.error("Error fetching Nest Nepal transactions", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="text-center text-sm text-[#0e0e0e]/40 py-8">
        Loading Nest Nepal sales...
      </div>
    );
  }

  return <SourceColumn source="nest" transactions={transactions} />;
}
